...
import useGetWeeklyRevenue from "@/customHooks/admin/useGetWeeklyRevenue";
...
export default function WeeklyRevenue() {
  const { data, isLoading } = useGetWeeklyRevenue();
  ...
  useEffect(() => {
    if (data) {
      ...
      const listKey = data.map((item) => item.currentDate);
  ...
  const handleClickDownloadChart = async () => {
    if (chartRef.current) {
      ...
      link.download = `weekly_revenue_chart_${currentTimeStamp}.svg`;
  ...
  return (
    <>
      {isLoading && (
        <>
          <span className="income-text">Doanh thu tuần</span>
          <LoadingContent />
        </>
      )}
      {!isLoading && data && (
